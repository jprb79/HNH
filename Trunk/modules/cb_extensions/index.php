...
function listAgent($pDB, $smarty, $module_name, $local_templates_dir)
{
    ...
    foreach ($listaAgentes as $tuplaAgente) {
        $tuplaData = array(
			...
			htmlentities($tuplaAgente['office'], ENT_COMPAT, 'UTF-8'),
            "<a href='?menu=$module_name&amp;action=edit_agent&amp;id_agent=" . $tuplaAgente["number"] . "'>[Sửa]</a>",
        );
/*
...
*/
    ...
    $arrGrid = array("title"    => "Số máy nhánh",
                     "url"      => $url,
                     "url"      => $url,
    ...
    $smarty->assign(array(
        ...
        'LABEL_CREATE_AGENT'    =>  "Thêm tổng đài viên",
...
function formEditAgent($pDB, $smarty, $module_name, $local_templates_dir, $id_agent)
{
    ...
    $contenidoModulo = $oForm->fetchForm(
        ...
        is_null($id_agent) ? "Thêm mới tổng đài viên" : _tr('Edit agent').' "'.$_POST['description'].'"',
        $_POST);
    return $contenidoModulo;
}
...
function getFormAgent(&$smarty, $oAgentes, $arrAgente, $bEdit)
{
    ...
    $smarty->assign("CANCEL", "Hủy");
    $smarty->assign("APPLY_CHANGES", "Lưu");
    ...
    // get list of office
    $arrOffice = $oAgentes->getOfficeList();
    $arrFormElements = array(
        "description" => array(
            ...
            "REQUIRED"               => "yes",
            "INPUT_TYPE"             => "TEXT",
            "INPUT_EXTRA_PARAM"      => array("id"=>"name"),
