...
function listen(uniqueid,filename,calldate,src,channel,dst,dstchannel,billsec,dcontext,userfield2)
{
    $.post('index.php?menu=' + module_name + '&rawmode=yes', {
        ...
        function (response) {
            $('#player_dialog').empty();
            $('#player_dialog').html(response);
            $('#src').html(src);
            $('#channel').html(channel);
            $('#dst').html(dst);
            $('#dstchannel').html(dstchannel);
            $('#calldate').html(calldate);
            $('#billsec').html(billsec);
