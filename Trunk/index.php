...
// auto login to agent_console - Tri Do
if(isset($_POST['submit_login_2'])) {
    $pass_md5 = md5('agent@123');
    if($pACL->authenticateUser('agent', $pass_md5)) {
        $_SESSION['elastix_user'] = 'agent';
        $_SESSION['elastix_pass'] = $pass_md5;
        header("Location: index.php");
        writeLOG("audit.log", "LOGIN agent: Web Interface login successful. Accepted password for agent from $_SERVER[REMOTE_ADDR].");
        exit;
    } else {
        $user = urlencode(substr('agent',0,20));
        if(!$pACL->getIdUser('agent')) // not exists user?
