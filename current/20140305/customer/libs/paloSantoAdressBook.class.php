...
*/
    ...
    function addContact($data,$elastix_user,$agent_add=false)
    {
        // check existence of the numbers
        foreach ($data['customer_phone'] as $row){
            $result = $this->check_phone_duplicate($row);
            if (!$result){
    ...
    private function check_phone_duplicate($number)
    {
        try {
            $result = $this->_DB->getFirstRowQuery("SELECT customer_code, customer_name FROM customer_contact a
                LEFT JOIN customer b ON a.customer_id = b.id
                WHERE a.phone = '%$number'", true);
            if(!$result && $result==null && count($result) < 1) {
                return false;
            }
            else {
    ...
    function updateContact($data,$id,$elastix_user,$agent_create=false)
    {
        // check existence of the numbers
        foreach ($data['customer_phone'] as $row){
            $result = $this->check_phone_duplicate($row);
