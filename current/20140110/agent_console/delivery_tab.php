...
$sortname = isset($_POST['sortname']) ? $_POST['sortname'] : 'purchase_date';
...
foreach($data as $row){
    ...
    $entry = array(
            ...
            'cell'=>array(
				...
				'purchase_date'=>isset($row['purchase_date']) ? date("d-m-Y h:i:s",strtotime($row['purchase_date'])) : '',
				'ticket_code'=>$code_list,
				'action'=>$view.($row['status']=='Mới'||$row['status']=='Chờ xử lý'?$edit.'&nbsp'.$disable:''),
        ),
    );
    $jsonData['rows'][] = $entry;
}
