...
SELECT agent.number, agent.name, call_entry.datetime_entry_queue AS start_date,
    call_entry.datetime_end AS end_date, call_entry.duration, 
    call_entry.duration_wait, queue_call_entry.queue, 'Inbound' AS type,
    call_entry.callerid AS telefono,
    call_entry.transfer, call_entry.status, call_entry.id as call_entry_id,
	call_entry_note.note, ticket_delivery.id as delivery
FROM (call_entry, queue_call_entry)
LEFT JOIN agent
    ON agent.id = call_entry.id_agent
LEFT JOIN call_entry_note
	ON call_entry_note.call_entry_id = call_entry.id
LEFT JOIN ticket_delivery
	ON ticket_delivery.call_entry_id = call_entry.id		
...
SQL_INCOMING;
		...
		$sPeticion_incoming .= ' ORDER BY call_entry.datetime_entry_queue';
...
SELECT agent.number, agent.name, calls.start_time AS start_date,
    ...
    calls.transfer, calls.status, '-' as call_entry_id,
	'' as note, '' as delivery
FROM (calls, campaign)
LEFT JOIN agent
...
SQL_OUTGOING;
        ...
        $sPeticionSQL .= ' ORDER BY start_date';
