...
function _moduleContent(&$smarty, $module_name)
{
            ...
            if (!is_array($recordset)) {
                ...
                function _calls_detail_map_recordset($cdr)
                {
                    ...
                    return array(
						...
						((is_null($cdr[12])||trim($cdr[12]==''))?'':'<a href="javascript:void(0)" onclick="view_note(\''.$cdr[11].'\')">Xem</a>'),
						((is_null($cdr[13])||trim($cdr[13]==''))?'':'<a href="javascript:void(0)" onclick="view_delivery(\''.$cdr[13].'\')">Xem</a>'),
                    );
                }
    ...
    $arrColumnas = array('STT','Số Agent', 'Nhân viên', _tr("Start Date"),
        _tr("Start Time"), _tr("End Date"), _tr("End Time"), "Thời lượng",
        _tr("Thời gian chờ"), _tr("Queue"), _tr("Số điện thoại"),
        _tr("Chuyển máy"), _tr("Status"),'Nội dung','Giao vé');
