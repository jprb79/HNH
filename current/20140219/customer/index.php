...
function report_adress_book($smarty, $module_name, $local_templates_dir, $pDB, $pDB_2, $arrLang, $arrConf, $dsn_agi_manager, $dsnAsterisk)
{
    ...
    if($oGrid->isExportAction()){
        ...
        $arr_cols = array(
            0 => array("name"      => "Mã KH",
                "property1" => ""),
            1 => array("name"      => "Tên khách hàng",
                "property1" => ""),
            2 => array("name"      => "Họ/Hình thức",
                "property1" => ""),
            3 => array("name"      => "Điện thoại/Liên hệ",
                "property1" => ""),
            4 => array("name"      => "Email/Liên hệ",
                "property1" => ""),
            5 => array("name"      => "Booker",
                "property1" => ""),
            6 => array("name"      => "Kinh doanh",
                "property1" => ""),
            7 => array("name"      => "Kế toán",
                "property1" => ""),
            8 => array("name"      => "Phân loại",
                "property1" => ""),
            9 => array("name"      => "Thẻ thành viên",
                "property1" => ""),
            10 => array("name"      => "Cách thanh toán",
                "property1" => ""),
        );
    }
    else{
        ...
        $arr_cols = array(
            ...
            2 => array("name"      => "Tên khách hàng",
                "property1" => ""),
            3 => array("name"      => "Họ/Hình thức",
                "property1" => ""),
            4 => array("name"      => "Điện thoại/Liên hệ",
                "property1" => ""),
    ...
    if(is_array($arrResult) && $total>0){
        if($oGrid->isExportAction())
            foreach($arrResult as $key => $adress_book){
                ...
                $arrTmp[]  = $adress_book['firstname']; //Ten column
                $arrTmp[]  = $adress_book['lastname']; //Ten column
                $arrTmp[]  = $phone_list;
                $arrTmp[]  = $email_list;
            ...
            foreach($arrResult as $key => $adress_book){
                ...
                if (isset($adress_book['number']) && count($adress_book['number'])>0)
                    foreach ($adress_book['number'] as $phone) {
                        ...
                        $call = trim($phone1[0]);
                        $phone_list .= is_null($call)||trim($call)==''?'':"<a href='?menu=$module_name&action=call2phone&id=".$call."'><img border=0 src='/modules/$module_name/images/call.png' title='Gọi số ".$call."'/></a> ".$phone.'<br/>'; //SDT column
                    }
