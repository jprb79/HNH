...
*/
    ...
    function getAddressBook($limit=NULL, $offset=NULL, $field_name=NULL, $field_pattern=NULL, $count=FALSE, $sortname='firstname asc',$letter=null)
    {
        // SIEMPRE se debe filtrar por usuario activo. Véase bug Elastix #1529.
    	$sql = 'SELECT '.($count ? 'COUNT(*) AS total' : '*').' FROM customer';
        $whereFields = null;
        $sqlParams = null;
        ...
        if (!is_null($field_name) and !is_null($field_pattern)) {
            ...
            if ($field_name=='phone'){
                $customer_search = $this->_DB->fetchTable("SELECT customer_id from customer_phone
                  WHERE number like '$field_pattern'", true);
                $contact_search = $this->_DB->fetchTable("SELECT customer_id from customer_contact
                  WHERE phone like '$field_pattern'", true);
                $customer_search = array_merge($customer_search,$contact_search);
            ...
            elseif ($field_name=='email'){
                $customer_search = $this->_DB->fetchTable("SELECT id as customer_id from customer
        ...
        foreach ($result as $row) {
            ...
            if ($row['type']=='0' || $row['type']=='1'){
                $arr_phone = $this->_DB->fetchTable("SELECT number from customer_phone WHERE customer_id=$id", true);
                $index_phone = 0;
                foreach ($arr_phone as $row_phone){
                    $result[$index]['number'][$index_phone] = $row_phone['number'];
                    $index_phone++;
                }
                $result[$index]['email'] = explode('\n',$result[$index]['email']);
            }
            else{
                $arr_contact = $this->_DB->fetchTable("SELECT * from customer_contact WHERE customer_id=$id", true);
                $index_phone = 0;
                $index_email = 0;
                $result[$index]['number'] = array();
                $result[$index]['email'] = array();
                foreach ($arr_contact as $row_contact){
                    if (!is_null($row_contact['phone'])&&trim($row_contact['phone'])!='') {
                        $result[$index]['number'][$index_phone] = $row_contact['phone'].' - '.$row_contact['name'];
                        $index_phone++;
                    }
                    if (!is_null($row_contact['phone'])&&trim($row_contact['phone'])!='') {
                        $result[$index]['email'][$index_email] = $row_contact['email'].' - '.$row_contact['name'];
                        $index_email++;
                    }
                }
            }
            // agent insert
            $result[$index]['booker'] = $this->getBookerName($row['agent_id']);
            // payment_type insert
            $result[$index]['payment'] = $this->getPaymentTypeName($row['payment_type']);
            $index++;
        }
        return $result;
    }
    ...
    function contactData($id)
    {
        $where = "a.id=?";
        $params = array($id);
        $query   = "SELECT a.*, b.name as agent, c.name as payment FROM customer a LEFT JOIN agent b ON a.agent_id = b.id
        ...
        if(!$result && $result==null && count($result) < 1)
            ...
            if ($type=='0' || $type=='1'){
                ...
                $index_phone = 0;
                foreach ($arr_phone as $row_phone){
                    $result['number'][$index_phone] = $row_phone['number'];
                    $index_phone++;
                }
            }
            else{
