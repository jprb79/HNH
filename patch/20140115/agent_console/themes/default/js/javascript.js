...
$(document).ready(function() {
    ...
    $('#btn_password').click(do_open_password);
...
function do_login()
{
	$.post('index.php?menu=' + module_name + '&rawmode=yes', {
		...
		ext_callback: 	'SIP/' + $('#input_extension_callback').attr('value'),
...
function do_open_password()
{
    $('#txt_old_password').val('');
    $('#txt_new_password').val('');
    $('#txt_new_password_2').val('');
...
{
    // validate password
    $('#waiting_text').html('Vui lòng chờ...');
    $('#waiting').dialog('open');
    $.post('index.php?menu=' + module_name + '&rawmode=yes', {
            menu:		module_name,
            rawmode:	'yes',
            action:		'changePassword',
            agent_extension:       $('#agent_extension').val(),
            old_password:   $('#txt_old_password').val(),
            new_password:   $('#txt_new_password').val(),
            new_password2:  $('#txt_new_password_2').val()
        },
        function (response) {
            ...
            if (response['action'] == 'error')
                ...
                alert('Đổi mật khẩu thành công. Vui lòng đặng nhập lại với mật khẩu mới...');
                $('#dialog_change_password').dialog('close');
            }
        })
        .fail(function() {
            alert('Kết nối máy chủ thất bại! Liên hệ admin.');
        });
}
// end of change password for agent
...
function do_view_delivery(delivery_id)
{
    $.post('index.php?menu=' + module_name + '&rawmode=yes', {
    ...
    function (response) {
        ...
        if (response['action'] == 'error') {
            ...
            view_delivery_detail(response['message']);
        }
    });
}

function show_outgoing_info(number)
{
    $.post('index.php?menu=' + module_name + '&rawmode=yes', {
            action:		'callOut',
            number:	number
        ...
        function (response) {
            //console.log(response);
            manejarRespuestaStatus(response);
        });
}
...
function make_call(number)
{
    ...
    function (response) {
        ...
        if (response['action'] == 'error')
            ...
            show_outgoing_info(response['message']);
    });
}
...
function do_checkstatus()
{
	...
	if (window.EventSource) {
		...
		evtSource.onmessage = function(event) {
            ...
            $('#elastix-callcenter-error-message-text').hide();
		}
		evtSource.onerror = function(event) {
            //console.log(evtSource.onerror);
            $('#elastix-callcenter-error-message-text').text('Mất kết nối đến máy chủ, vui lòng bấm F5 hoặc chờ đợi...');
            $('#elastix-callcenter-error-message-text').show();
            // debug long responding
            setTimeout(do_checkstatus, 500);
		}
	} else {
...
function manejarRespuestaStatus(respuesta)
{
	for (var i in respuesta) {
		...
		case 'holdexit':
			...
			break;
		case 'agentlinked':
            // Fix bug load dialog 2 lan
            $("#ticket-delivery").dialog('destroy');
            $("#ticket-delivery").remove();
		...
		case 'agentunlinked':
			...
			$('#transfer').hide();
			$('#clicktocall').attr("style","display:inline-block");
...
function mostrar_mensaje_info(s)
{
    ...
    $('#elastix-callcenter-info-message-text').show();
...
function chkDelivery(select)
{
	if(select.checked) {
		$('#ticket-delivery').fadeIn('slow');
        $('#deliver-price').focus();
    }
    else
		$('#ticket-delivery').fadeOut('slow');
}