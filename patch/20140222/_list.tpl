<form  method="POST" style="margin-bottom:0;" action="{$url}">
    ...
    <tr>
        <td>
        <table {if $TABLE_NAME !=""}id="{$TABLE_NAME}"{/if} class="table_data" align="center" cellspacing="0" cellpadding="0" width="100%">
            ...
            {foreach from=$arrData key=k item=data name=filas}
                ...
                {else}
                    <tr {if $TABLE_NAME !=""}id={$TABLE_NAME}_{$data[0]}{/if} onMouseOver="this.style.backgroundColor='#f2f2f2';" onMouseOut="this.style.backgroundColor='#ffffff';">
                        {if $smarty.foreach.filas.last}
                            {section name=columnNum loop=$numColumns start=0 step=1}
                            <td valign="top" class="table_data_last_row">{if $data[$smarty.section.columnNum.index] eq ''}&nbsp;{/if}{$data[$smarty.section.columnNum.index]}</td>
                            {/section}
                        {else}
                            {section name=columnNum loop=$numColumns start=0 step=1}
                            <td valign="top" class="table_data">{if $data[$smarty.section.columnNum.index] eq ''}&nbsp;{/if}{$data[$smarty.section.columnNum.index]}</td>
                            {/section}
                        {/if}
                    </tr>
                {/if}
