...
(function ($) {
	...
	$.addFlex = function (t, p) {
		...
		p = $.extend({ //apply default properties
			...
			title: false,
			idProperty: 'id',
			pagestat: 'Đang xem từ {from} đến {to} của {total} mục',
			pagetext: 'Trang',
			outof: 'của',
			findtext: 'Tìm theo',
			params: [], //allow optional parameters to be passed around
			procmsg: 'Đang xử lý. Vui lòng đợi ...',
			query: '',
			qtype: '',
			nomsg: 'Danh sách rỗng',
		...
		if (p.usepager) {
			...
			if (p.searchitems) {
				...
				$(g.sDiv).append("<div class='sDiv2'>" + p.findtext +
						...
						" <input type='button' value='Tìm'/>&nbsp;<input type='button' value='Xóa' /></div>");
                ...
                $('input[value=Tìm]', g.sDiv).click(function () {
                    g.doSearch();
                });
				$('input[value=Xóa]', g.sDiv).click(function () {
