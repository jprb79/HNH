    ...
    $.post('index.php?menu=' + module_name + '&rawmode=yes', {
        ...
        function (response) {
            if (response['action'] == 'error') {
                ...
                refresh_call_history(response['message'],response['agent']);
            }
        });
}

function refresh_call_history(data,data2)
{
    /*
    // tab 1
    $('#call_history_1').empty();
    content = '<tr><th>Khách hàng</th><th>Giờ gọi</th><th>Tổng đài viên</th><th>Tình trạng</th><th>Nội dung</th><th>Giao vé</th></tr>';
    ...
    $('#call_history_1').append(content); */
    // tab 2
    $('#call_history_2').empty();
    content = '<tr><th>Khách hàng</th><th>Giờ gọi</th><th>Tổng đài viên</th><th>Tình trạng</th><th>Nội dung</th><th>Giao vé</th></tr>';
    for (var i = 0; i < data2.length; i++) {
        content += '<tr>';
        content += "<td>" + data2[i].phone + "</td>";
        content += "<td>" + data2[i].calldate + "</td>";
        content += "<td>" + data2[i].agent + "</td>";
        content += "<td>" + data2[i].status + "</td>";
        //console.log(data[i].note);
        if (data2[i].note != null)
            content += "<td><a href=\"javascript:void(0)\" onclick=\"view_note('" + data2[i].id + "')\">Xem</a></td>";
        else if (data2[i].permit_add == 1)
            content += "<td><a href=\"javascript:void(0)\" onclick=\"add_note_history('" + data2[i].id + "')\">Thêm</a></td>";
        else
            content += "<td>  </td>";
        if (data2[i].delivery != null)
            content += "<td><a href=\"javascript:void(0)\" onclick=\"view_delivery('" + data2[i].delivery_id + "')\">Xem</a></td>";
        else
            content += "<td>  </td>";
        content += '</tr>';
    }
    $('#call_history_2').append(content);
}
...
function view_customer(customer_id)
{
    ...
    $.post('index.php?menu=' + module_name + '&rawmode=yes', {
        ...
        function (response) {
            //console.log(response);
            $('.view_customer_fullname').val(response['customer_name']);
            $('.view_customer_customer_code').val(response['customer_code']);
            $('.view_customer_booker').val(response['booker']);
            $('.view_customer_sale').val(response['sale']);
            $('.view_customer_accoutant').val(response['accountant']);
            $('.view_customer_phone').val();
            var phone = '';
            for (var i=0;i<response['contact'].length;i++)
                phone += response['contact'][i]['phone']+"\n";
            $('.view_customer_phone').html(phone);
            $('.view_customer_membership').html(response['membership']);
            ...
            $('#view_customer_passport').val(response['passport']);
            $('#latest_note').html(response['latest_note']);
...
{
    $('#sabre').html(data['sabre']);
    $('#bsp').html(data['bsp']);
    $('#lion_air').html(data['lion_air']);
    $('#air_asia').html(data['air_asia']);
    $('#lao_airlines').html(data['lao_airlines']);
    $('#transviet').html(data['transviet']);
    $('#note').html(data['note']);
