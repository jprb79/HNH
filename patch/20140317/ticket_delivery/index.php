...
function CashCollection(&$pDB,$pDB_2)
{
    ...
    if (is_null(getParameter('unpaid'))) {
        $response = array(
            'action'    =>  'collect',
            'message'   =>  'Đã nhận tiền từ ID giao vé số ' . $sTicketId,
        );
        ...
        $response = array(
            'action'    =>  'uncollect',
            'message'   =>  'Đã hủy nhận tiền từ ID giao vé số ' . $sTicketId,
        );
...
function TicketProcess(&$pDB)
{
    ...
    switch ($sType) {
        case 'return':
            $response = array(
                'action'    =>  'return',
                'message'   =>  'ID giao vé số '.$sTicketId.' chuyển sang trạng thái chờ xử lý! ',
            );
            break;
        case 'enable':
            $response = array(
                'action'    =>  'enable',
                'message'   =>  'Đã tạo lại ID giao vé số '.$sTicketId.' thành công!',
            );
            break;
        case 'disable':
            $response = array(
                'action'    =>  'disable',
                'message'   =>  'Đã hủy ID giao vé số '.$sTicketId.' thành công! ',
            );
            break;
        case 'wait':
            $response = array(
                'action'    =>  'wait',
                'message'   =>  'Đã chuyển trạng thái "Chờ giao" cho ID giao vé số '.$sTicketId.' thành công! ',
            );
            break;
        default:
            break;
...
function report_TicketDelivery($smarty, $module_name, $local_templates_dir, &$pDB,$pDB_2)
{
    ...
    if(is_array($arrResult) && $total>0){
        foreach($arrResult as $key => $value){
            ...
            $arrTmp[9] = (is_null($value['delivery_date'])?'':date("d-m-Y H:i:s",strtotime($value['delivery_date'])));
            $arrTmp[10] = $download;
            $arrTmp[11] = (is_null($value['collection_date'])?'':date("d-m-Y H:i:s",strtotime($value['collection_date'])));
            $arrTmp[12] = $function;
...
function updateRow_TicketDelivery($pDB,$pDB_2,$module_name)
{
    ...
    $html .= '<td class="table_data">'.(is_null($value['delivery_date'])?'':date("d-m-Y H:i:s",strtotime($value['delivery_date']))).'</td>';
    $html .= '<td class="table_data">'.$download.'</td>';
    $html .= '<td class="table_data">'.(is_null($value['collection_date'])?'':date("d-m-Y H:i:s",strtotime($value['collection_date']))).'</td>';
...
function assign_TicketDelivery(&$pDB)
{
    ...
    $response = array(
        'action'    =>  'assign',
        'message'   =>  'Phân công ID giao vé ' . $sTicketId . ' thành công',
    );
