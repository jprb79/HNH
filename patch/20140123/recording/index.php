...
function reportMonitoring($smarty, $module_name, $local_templates_dir, &$pDB, $pACL, $arrConf, $user, $extension, $esAdministrador)
{
    ...
    if($oGrid->isExportAction()){
        ...
        $oGrid->deleteList(_tr("message_alert"),'submit_eliminar',_tr("Delete"));
            ...
            $buttonDelete = "";
        */
        $arrColumns = array($buttonDelete, "Ngày", "Giờ", "Gọi từ", "Gọi đến","Thời gian gọi",
        ...
        if(is_array($arrResult) && $total>0){
            ...
            foreach($arrResult as $key => $value){
                ...
                $arrTmp[8] = (is_null($value['note_id'])?'-':'<a href="javascript:void(0)" onclick="view_note(\'' . $value['note_id'] . '\')">Xem</a>');
                $arrTmp[9] = (is_null($value['delivery_id'])?'-':'<a href="javascript:void(0)" onclick="view_delivery(\'' . $value['delivery_id'] . '\')">Xem</a>');
                $arrData[] = $arrTmp;
            }
        }
    }
